...
@Deprecated
public class TCPEsLogger implements XcsLogger, CallBackAble, LifeCycle, Configurable {
    ...
    public void start() {
        ...
        started = true;
        if (changer == null)
            changer = ElasticsearchLogChanger.getChanger(null);
        if (sender == null) {
            sender = SenderFactory.getSender(cls,null);
        }
    ...
    public void stop() {
        ...
        started = false;
        if (sender != null && LifeCycleUtils.started(sender)) {
            LifeCycleUtils.stop(sender);
        }
    }
