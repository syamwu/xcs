package syamwu.xchushi.easylog.elasticsearch.logger;
...
import syamwu.xchushi.easylog.elasticsearch.changer.Changer;
import syamwu.xchushi.easylog.elasticsearch.changer.ElasticsearchLogChanger;
 ...
 * @date: 2018
 */
public class ElasticSearchSubjectLogger extends AbstractSubject<String> implements XcsLogger, LifeCycle, Configurable {
    ...
    public void append(LoggerType loggerType, Thread thread, StackTraceElement st, String message, Throwable t,
        ...
        try {
            ...
            offer(changer.logString(loggerMap));
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
    }
    ...
    public synchronized void start() {
        ...
        started = true;
        if (configure == null) {
            configure = FactoryProxy.getFactory(Configure.class, EasyLogProxy.getInstance(), true)
                    .getInstance(getClass());
        }
        if (changer == null) {
            changer = new ElasticsearchLogChanger();
        ...
        if (observer != null) {
            ...
            LifeCycleUtils.start(observer, false);
        }
    }
